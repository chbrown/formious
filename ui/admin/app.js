...
export class AppLayout extends React.Component {
  render() {
    return (
      <div>
        <nav fixedflow>
          <NavLink to="/admin/aws_accounts">AWS Accounts</NavLink>
          <NavLink to="/admin/mturk">MTurk</NavLink>
          <NavLink to="/admin/administrators">Administrators</NavLink>
          <NavLink to="/admin/experiments">Experiments</NavLink>
          <NavLink to="/admin/templates">Templates</NavLink>
          <NavLink to="/admin/responses">Responses</NavLink>
          <div style="float: right">
            {/*<a ui-sref="admin.administrators.edit({id: 'me'})" ui-sref-active="current" className="tab">me</a>*/}
            <button ng-click="logout($event)" className="anchor tab">Logout</button>
          </div>
        </nav>
        {children}
      </div>
...
export class Login extends React.Component {
  render() {
    return (
      <div>
        <div style={width: '200px', margin: '80px auto', backgroundColor: 'white'} className="shadow">
          <form ng-submit="login(email, password)" style="padding: 10px">
            <h3>Admin Login</h3>

            <label>
              <div>Email</div>
              <input ng-model="email" style="width: 100%" />
            </label>

            <label>
              <div>Password</div>
              <input ng-model="password" type="password" style="width: 100%" />
            </label>
...
/**
<span className="help">This is a long but very useful help message but most often you probably won't want to read all of it because it's actually easy to remember and not all that helpful. Good for reference though.</span>
...
export class Help extends React.Component {
  ...
  render() {
    return (
      <span className="help">
        <span className="summary" style="opacity: 0.5" ng-hide="expanded" ng-click="expanded = true"></span>
        <span className="full" ng-show="expanded" ng-click="expanded = false" ng-transclude></span>
      </span>
    );
  }
}
