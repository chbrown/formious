(ns formious.admin.pages.qualification-types
...
(rum/defc QualificationTypeEdit
  ...
  [:div
   ...
   [:label {:className "block"}
    [:div
     [:b "Status"]
     [:span {:className "help"} "The initial status of the Qualification type."]]
...
(rum/defc QualificationTypeNew
  []
  [:div
   [:section {:className "hpad"} [:h3 "CreateQualificationType"]]
   [:section {:className "box hpad"}
  ...
  [:div
   ...
   [:section {:className "box hpad"}
    [:form {:ng-submit "sync($event)"}
     [:ng-include {:src "'/ui/admin/mturk/qualification_types/form.html'"}]
     [:div {:className "block"} [:button "Submit"]]]]
   [:section {:className "hpad"} [:h3 "Qualifications"]]
   [:section {:className "box"}
    [:table {:className "striped lined padded fill"}
     [:thead
